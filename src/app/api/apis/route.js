...
export async function GET(req) {
  try {
    ...
    if (id) {
      // ✅ Fetch specific API with category name
      const [rows] = await pool.query(`
        SELECT api_v2.*, categories.name AS category_name
        FROM api_v2
        LEFT JOIN categories ON api_v2.category_id = categories.id
        WHERE api_v2.id = ?
      `, [id]);
      ...
      const api = {
        ...
        request_example: rows[0].request_example || "{}", // ✅ Keep raw JSON
        response_body: JSON.parse(rows[0].response_body || "[]"),
        response_example: rows[0].response_example || "{}", // ✅ Keep raw JSON
    ...
    // ✅ Fetch all APIs with category names
    const [rows] = await pool.query(`
      SELECT api_v2.*, categories.name AS category_name
      FROM api_v2
      LEFT JOIN categories ON api_v2.category_id = categories.id
    ...
    const apis = rows.map((api) => ({
      ...
      request_example: api.request_example || "{}", // ✅ Keep raw JSON
      response_body: JSON.parse(api.response_body || "[]"),
      response_example: api.response_example || "{}", // ✅ Keep raw JSON
...
export async function POST(req) {
  try {
    ...
    if (!validMethods.includes(method.toUpperCase())) {
      return new Response(JSON.stringify({ error: "Invalid HTTP method" }), { status: 400 });
    }
    ...
    const [result] = await pool.query(
      ...
      [name, endpoint, method.toUpperCase(), description, category_id, schema_id, JSON.stringify(headers || []), JSON.stringify(query_params || []), JSON.stringify(request_body || []), sanitizedRequestExample, JSON.stringify(response_body || []), sanitizedResponseExample, JSON.stringify(response_codes || []), curl_example]
    );
...
export async function PUT(req) {
  try {
    ...
    await pool.query(
      ...
      [name, endpoint, method.toUpperCase(), description, category_id, schema_id, JSON.stringify(headers || []), JSON.stringify(query_params || []), JSON.stringify(request_body || []), sanitizedRequestExample, JSON.stringify(response_body || []), sanitizedResponseExample, JSON.stringify(response_codes || []), curl_example, id]
    );
